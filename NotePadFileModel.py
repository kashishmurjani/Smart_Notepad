...
class File_Model:
    def __init__(self):
        self.url = ""
        self.key = string.ascii_letters + ''.join(str(x) for x in range(10))
        self.offset = 5
    ...
    def save_as(self, msg):
        ...
        self.url = filedialog.asksaveasfile(mode="w", filetypes=[("Encrypted files", "*.ntxt")],
    ...
    def save_file(self, msg):
        if self.url == "":
            self.url = filedialog.asksaveasfilename(title="Select file name", filetypes=[("Text document", "*.*")],
