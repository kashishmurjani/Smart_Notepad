...
class Notepad:
    def __init__(self):
        self.root = tk.Tk()
        self.root.geometry('1200x750+50+10')
    ...
    def check_db_status(self):
        ...
        if self.db_status:
            ...
            messagebox.showinfo("Database Error", "Can not connect to the DB")

    def exit_func(self, event=None):
        try:
            if self.text_changed:
                answer = messagebox.askyesno("File not saved", "Do you want to save the changes?")
                if answer:
                    contents = self.text_editor.get(1.0, tk.END)
                    self.file_controller.save_file(contents)
        except:
            messagebox.showerror("Application error", "Error in closing")
    ...
    def save_file(self, event=None):
        try:
            ...
            messagebox.showerror("File Error", "Can not save the file")
            print(traceback.format_exc())

    def save_as(self, event=None):
        try:
            contents = self.text_editor.get(1.0, tk.END)
            self.file_controller.save_as(contents)
        except:
            messagebox.showerror("File Error", "Can not save the file")
            print(traceback.format_exc())
    ...
    def say_something(self):
        try:
            self.take_audio = self.file_controller.takeQuery()
            print(self.take_audio)
            if self.take_audio.lower() == "hey mojo open file":
                self.open_file()
            elif self.take_audio.lower() == "hey mojo open new file":
                self.new_file()
            elif self.take_audio.lower() == "hey mojo save file":
                self.save_file()
            elif self.take_audio.lower() == "hey mojo save as new file":
                self.save_as()
            elif self.take_audio.lower() == "hey mojo close app":
                self.exit_func()
            elif self.take_audio.lower() == "hey mojo text bold":
                self.change_bold()
            elif self.take_audio.lower() == "hey mojo text italic":
                self.change_italic()
            elif self.take_audio.lower() == "hey mojo search":
                self.find_func()
            elif self.take_audio.lower() == "hey mojo hide statusbar":
                self.hide_statusbar()
            elif self.take_audio.lower() == "hey mojo hide toolbar":
                self.hide_toolbar()
            else:
            ...
            print(traceback.format_exc())
        except WaitTimeoutError as unkwe:
            messagebox.showerror("Wait time out error", "Time limit exceeded")
            print(traceback.format_exc())
        except UnknownValueError as wte:
            messagebox.showerror("Unknown Value error", "Speech unrecognisable")
            print(traceback.format_exc())
    ...
    def secure_file(self, event=None):
        if self.db_status:
            ...
            # self.replace_button = ttk.Button(self.file_frame, text="Replace",command=self.replace)
            ...
            # self.fileList.place(relx=0.9, rely=0.30, retheight =1, relwidth =8.972)
            ...
            self.fileList.configure(highlightcolor="#090909")
    ...
    def open_secure_file(self):
        try:
            ...
            if self.sel_file_index_tuple == 0:
    ...
    def remove_secure_file(self):
        try:
            ...
            if len(self.sel_file_index_tuple) != 0:
                ...
                messagebox.showerror("Error", "Please select file first")
        except:
            messagebox.showerror("Secure file error", "Error while opening file")
            print(traceback.format_exc())
    ...
    def add_file(self):
        try:
            if self.file_owner_input.get().strip() != "" and self.file_pwd_input.get().strip() != "":
                ...
                messagebox.showerror("Error", "Please input file owner and password before adding the file")
        except:
    ...
    def set_time_counter(self):
        self.current_time_counter = ttk.Label(self.root, text=f'Current time:', foreground="cyan", background="black")
        self.current_time_counter.place(x=1030, y=0)
        self.time_counter1 = ttk.Label(self.root, foreground="black", background="cyan")
        self.time_counter1.place(x=1110, y=0)
        self.execution_time_counter = ttk.Label(self.root, text=f'Execution time:', foreground="cyan",
                                                background="black")
        self.execution_time_counter.place(x=1030, y=20)
        self.time_counter2 = ttk.Label(self.root, foreground="black", background="cyan")
        self.time_counter2.place(x=1120, y=20)

    def run(self):
        self.root.mainloop()

class CurrentTime(Thread):
    def __init__(self,time_counter1):
        super().__init__()
        self.time_counter1=time_counter1
    def run(self):
        string = strftime('%H:%M:%S %p')
        self.time_counter1.config(text=string)
        self.time_counter1.after(1000, self.run)


class ExecutionTime(Thread):
    def __init__(self, time_counter2):
        super().__init__()
        self.time_counter2 = time_counter2
        self.lastTime = ""
        t = localtime()
        self.startTime = dt.timedelta(hours=t[3], minutes=t[4], seconds=t[5])

    def run(self):
        now = dt.datetime.now()
        executedTime = now - self.startTime
        self.executedTime2 = executedTime.strftime('%H:%M:%S')
        if self.executedTime2 != self.lastTime:
            self.lastTime = self.executedTime2
            self.time_counter2.config(text=self.executedTime2)
        self.time_counter2.after(1000, self.run)


obj = Notepad()
thread1 = CurrentTime(obj.time_counter1)
thread2 = ExecutionTime(obj.time_counter2)
thread1.start()
thread2.start()
obj.run()
thread1.join()
thread2.join()
print(f"Execution time: {thread2.executedTime2} secs")
